if (!window.loaded_modss) {
    ...
    (function() {
        ...
        var TRASH_R = ['$$$####!!!!!!!', '^^^^^^##@', '@!^^!@#@@$$$$$', '^^#@@!!@#!$', '@#!@@@##$$@@'];
        var version_modss = '3.3', API = Protocol() + 'api.lampa.stream/', type = '', jackets = {}, cards, ping_auth, manifest, menu_list = [], vip = false, leftVipD = '', user_id = '', uid = 'dcbee9ef84465be64feb69380', IP, logged = undefined, cashe = encodeURIComponent(Lampa.Base64.encode(window.location.origin));
        ...
        console.log('Modss', 'init', 'LOADED - CA [137.184.174.93] - [GET] ' + urls[0]);
        ...
        function tryNextUrl() {
            if (currentUrlIndex >= urls.length) {
                ...
                console.log('Modss', 'init', 'CA [137.184.174.93] - все URL недоступны. Статусы ответов:', statusCodes, loadErrors);
                Lampa.Noty.show('CA [137.184.174.93] - MODSs ОШИБКА ЗАГРУЗКИ ПЛАГИНА -> все серверы недоступны.<br>Статусы: ' + (statusCodes || '—') + '<br>Lampa: ' + window.location.origin, {
                    time: 10000
                });
            ...
            network.silent(currentUrl, function(json) {
                ...
                console.log('Modss', 'init', 'CA [137.184.174.93] - load get plugin error - [POST] ' + currentUrl, 'status:', statusNum, errMsg);
                currentUrlIndex++;
                tryNextUrl();
            }, requestData, {
                dataType: 'text'
